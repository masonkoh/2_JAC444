...
public class BabyName implements Serializable {
	...
	public static void setYear(int fYear) {
		year = fYear;
	}
	...
	public static void findName(String name) {
		...
		try (BufferedReader br = new BufferedReader(new FileReader(fileName))) {
			...
			while ((line = br.readLine()) != null && matchFound == false) {
				...
				String[] l = line.split("\\s+"); // \\s+ means one more space than \\s
	...
	public static void main(String[] args) {
		...
		Pattern letters = Pattern.compile("\\p{Alpha}+"); // regex to check name input //\p{Alpha} An alphabetic character:[\p{Lower}\p{Upper}]
			...
			try {
				do {
					...
					if (Character.compare(keepRunning, 'N') == 0) {
						validEnd = false; // IT WORKS WITHOUT THIS LINE BUT I'M DOING IT ANYWAYS
						end = true;
						break;
					}
